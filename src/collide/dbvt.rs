...
/// * Only leaf nodes contain user data.
///
/// The main heuristic used for insertion and tree rotation is the surface area of the bounding
/// volumes.
///
/// # Type parameters:
///
        ...
        + SurfaceArea<Real>,
{
    ...
    fn rotate(&mut self, node_index: usize) -> Option<usize> {
        ...
        if !left_is_leaf || !right_is_leaf {
            ...
            if (my_surface_area - min_sa) / my_surface_area > 0.25 {
                match rot {
                    // do nothing
                    Rotation::None => (),
...
fn get_best_rotation<B>(
    ...
    left_index: usize,
    right_index: usize,
    node_surface_area: Real,
    left_is_leaf: bool,
    right_is_leaf: bool,
) -> (Rotation, Real)
where
    B: Union<B, Output = B> + SurfaceArea<Real>,
{
