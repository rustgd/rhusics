...
impl Primitive for Circle {
    fn get_far_point(&self,
        ...
        Point2::new(p.x + pose.position.x + body_offset.x, p.y + pose.position.y + body_offset.y)
    }

    fn get_bound(&self) -> Aabb2<f32> {
        Aabb2::new(Point2::new(-self.radius,
                               -self.radius),
                   Point2::new(self.radius,
                               self.radius))
    }
}
...
fn get_max_point(vertices: &Vec<Vector2<f32>>,
                 direction: &Vector2<f32>,
                 body_offset: &Vector2<f32>,
                 pose: &BodyPose) -> Point2<f32> {
    ...
    let (p, _) = vertices
        ...
        .fold((Vector2::new(0., 0.), std::f32::MIN),
              |(max_p, max_dot), (v, dot)| {
                  if dot > max_dot {
                      (v.clone(), dot)
                  } else {
